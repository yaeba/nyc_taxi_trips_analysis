...
trips_data[, 
  Airport_trip := Dropoff_location %in% airport_zones | RateCodeID %in% c(2, 3)]
...
trips_data %>%
  ...
  left_join(weather, by=c("Pickup_date" = "DATE")) %>%
  gather(AWND:AVG_T, key="Measure", value="Value") %>%
  ggplot(aes(x=Value, y=Total_amount, color=Season)) +
  geom_point() +
  geom_smooth(method=lm) +
  facet_wrap(Measure ~ ., scales="free_x")
...
trips_data %>%
  ...
  left_join(weather, by=c("Pickup_date" = "DATE")) %>%
  gather(AWND:AVG_T, key="Measure", value="Value") %>%
  ggplot(aes(x=Value, y=Trip_duration, color=Season)) +
  geom_point() +
  geom_smooth(method=lm) +
  facet_wrap(Measure ~ ., scales="free_x")
```

Days when raining or snowing, number of trips very different from regular days. Location? Under bridge or metro station? Plot on map
```{r}
